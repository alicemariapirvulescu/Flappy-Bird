...
(struct pasare (y v )  #:transparent)
(struct pipe (x y ) #:transparent)
(struct joc (pasare  pipes score) #:transparent )
...
(joc (pasare bird-initial-y 0)
            (list( pipe scene-width  (+ added-number (random random-threshold)) )) 0)
  ) 
...
(define (next-state-bird bird gravity)
   (pasare (+ (get-bird-y bird) (get-bird-v-y bird)) (+ (get-bird-v-y bird) gravity) ))
...
(define (next-state-bird-onspace bird momentum)
  (pasare (get-bird-y bird) (* -1 momentum)))
...
(define (change current-state pressed-key)
 (if (key=? pressed-key " ")
     (struct-copy joc current-state [pasare (next-state-bird-onspace (get-bird current-state) initial-momentum)])
   current-state))
...
; scroll-speed-ul dat.
 (define(edit-pipe scroll-speed)
   (lambda (p)
(pipe (- (get-pipe-x p) scroll-speed) (pipe-y p))))
 
(define (move-pipes listapipes scroll-speed)
  (map (edit-pipe scroll-speed)  listapipes))
...
(define (clean p)
  (if ( > (+ (get-pipe-x p) pipe-width) 0) true
      false))

(define (clean-pipes listapipes)
 (filter clean listapipes))
...
; cu pasărea.
(define (add-more-pipes listpipes)
 ( if (>= (length listpipes) no-pipes) listpipes
  (let ([p (last listpipes) ])
    (cons (pipe (+ (+  pipe-width  pipe-gap) (get-pipe-x p)) (+ added-number (random random-threshold))) listpipes)
    )))
...
(define (check-ground-collision bird)
 (if (> (+ (get-bird-y bird) bird-height) ground-y) true
     false))
...
(define (invalid-state? state)
  (if (or (check-ground-collision (get-bird state)) (check-pipe-collisions (get-bird state) (get-pipes state))) true
          false
  ))
...
(define (coliziune bird)
  (lambda(p)
 (if(or ( check-collision-rectangles (make-posn  bird-x (get-bird-y bird))
...
(define (check-pipe-collisions bird listpipes)
  (if(null? (filter (coliziune bird) listpipes)) #f
     #t))
...
( joc (next-state-bird (get-bird state) initial-gravity)  (next-state-pipes (get-pipes state) initial-scroll-speed) ( + (get-score state) 0.1) ))
...
(define (draw-frame state)
  ( let ( [ pipes-images (place-pipes (get-pipes state) initial-scene) ])
(place-image bird-image (+ bird-x (quotient  bird-width 2)) (+ (get-bird-y (get-bird state)) (quotient  bird-height 2))
             (place-image ground-image (quotient scene-width 2)  (- scene-height (quotient ground-height 2))
                          (place-image (score-to-image (get-score state)) text-x text-y pipes-images))) ))
; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipes pipes scene)

 (foldr 
   (lambda (p acc)
        (place-image gap  (+ (get-pipe-x p) (quotient pipe-width 2)) (+ (get-pipe-y p) (quotient pipe-self-gap 2 ))
                 (place-image pipe-image  (+ (get-pipe-x p) (quotient pipe-width 2)) (quotient scene-height 2 )acc)))
   scene pipes))
